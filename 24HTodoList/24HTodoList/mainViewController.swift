...
var datalist = [TodoVO]()
var list: [TodoVO] = { // 데이터 넣을 리스트 생성
    
    for (todoText, deadLineH, deadLineM, alarmCount) in dataset{ // 각 요소들을 하나하나 매칭하여 데이터 묶음 생성
        let tvo = TodoVO()
...
class MainViewController: UIViewController{
    ...
    @IBAction func doneBtn(_ sender: Any) { // 완료 버튼 누르면 새로운 dataset 생성 및 list에 추가
        ...
        let H: String = String(tvo.deadLineH!)
        let M: String = String(tvo.deadLineM!)
        tvo.deadLineString = H + " : " + M
        list.append(tvo)
